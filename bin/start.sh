...
if [ ! -d "$PWD/node_modules" ]; then
  echo "Please run the setup script from project's root folder"
  exit
fi
...
if [ ! $NODE_LOG_DIR ]; then
  export NODE_LOG_DIR="$NBS_CURR_PROJECT_PATH/log"
fi
if [ ! -d "$NODE_LOG_DIR" ]; then
  mkdir $NODE_LOG_DIR
fi

if [ ! -f "$NODE_LOG_DIR/websocket.log" ]; then
    touch $NODE_LOG_DIR/websocket.log
fi
