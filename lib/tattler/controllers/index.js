...
async function emit(query, callback)
{
	'use strict';
	...
	const instance = await tattler();

	if (instance.roomExists(root, query.room))
	{
		/** @namespace query.bag */
		...
		if (result.status === 200) 
		{
			try 
			{
				instance.io.sockets.in('/' + root + '/' + query.room).emit(event, query.bag);
...
exports.postRooms = async function (req, res)
{
	...
	t.makeClient(query.client.socketId, query.client.sessionId);
	
	var rooms = t.makeRooms(query.root, query.rooms, query.client.sessionId);
