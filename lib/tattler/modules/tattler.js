...
function isRoomInUse(socket, root, roomName)
{
	const rooms = socket.nsp.adapter.rooms;

	return rooms.hasOwnProperty('/' + root + '/' + roomName);
}

function deleteRoom(hall, roomName)
{
	redis.srem(hall, roomName);
}
			...
			var callbacks = {
						...
						if (sessionId !== null) {
							redis.get('tattler:roots:' + sessionId + ':' + roomName, function (err, root) {
								...
								if (root !== null) {
									...
									if (!joinedRooms[root]) joinedRooms[root] = [];
				...
				disconnect: function () {
					...
					for (var root in joinedRooms)
					{
						...
						{
							if (!isRoomInUse(socket, root, room)) {
								const hall = 'tattler:rooms:' + root;
								deleteRoom(hall, room);
							}
						}
					}
